...
def main():
    ...
    # Define a more complex set of granularities for LCPR_MatViT
    granularities = [
        {
            'vit_depth_scale': 0.5, 'vit_width_scale': 0.5,
        ...
        {
            'vit_depth_scale': 0.75, 'vit_width_scale': 0.75,
            'fusion_granularities': [
        ...
        {
            'vit_depth_scale': 1.0, 'vit_width_scale': 1.0,
            'fusion_granularities': [
    ...
    model = LCPR_MatViT.create(weights=ResNet18_Weights.DEFAULT)
    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')

    # The Trainer class in tools/runner.py needs to be modified to handle Matryoshka training.
    # The training loop should sample a random granularity from the list above at each step
    # and call `model.configure_subnetwork(granularity)` before the forward pass.
